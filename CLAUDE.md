...
* The extension is plain JavaScript with no build step (Sept 2026), matching `background.js` and `content-script.js`, so Chrome loads `extension/` directly. The popup uses one stylesheet, `popup.css`, with the palette defined as CSS variables at the top, instead of the app's inline-style approach. If a palette color changes, update both the app and `popup.css`.

## Design system

* Palette: Blue 400 `#378ADD` (primary actions/navigation), Green `#EAF3DE` bg / `#3B6D11` text / `#C0DD97` border (positive/add actions), Amber `#FAEEDA` bg / `#854F0B` text / `#FAC775` border (notes/warmth, and — as of Sept 2026 — extension store overrides), page background `#E6F1FB` (pale blue) — corrected Sept 2026; this was already the live background, "Gray 50 #F1EFE8" here was stale and undocumented
* Blue also carries a second meaning: "FitFile is pointing this out" (Sept 2026 revision) — now scoped to highlighted matches (e.g. a likely store match) and recommended-preset selection (e.g. "Everyday essentials"), not to overrides (see Amber, above). Highlighted matches use a left-edge accent bar (`border-left: 4px solid Blue 400` on a white card) rather than a fill, since a fill blended into the actual page background once it was corrected to `#E6F1FB`. Recommended-preset selection uses a solid Blue 400 fill with white text instead — unlike a passive highlighted-match row, a preset chip is an interactive selection control, so looking tappable is correct rather than a problem, and a solid fill isn't subject to the same background-blend issue a light fill has. The original shared Blue 50/200/800-900 fill recipe is retired for all three of its original uses (matches, overrides, presets). To verify at implementation: white text on solid Blue 400 measures 3.59:1, below the 4.5:1 threshold for small text. Weight 500 doesn't count as bold under WCAG, so the only fixes are text at 18pt (24px) or larger, which is likely too big for a chip in the popup, or a deeper blue for the preset fill.
...
* No-match / new-store fallback (Sept 2026): the store search list has a pinned "Core Sizes" option above the searchable list, for a store the user hasn't saved yet or one with nothing store-specific to show. It's a view state, not a saved store — selecting it doesn't touch `fitfile_stores`, consistent with the extension only surfacing what's already saved rather than creating stores.
* Store search behavior (Aug 2026): tapping "match a store" always opens the full searchable list, never jumps straight into a suggested store's data. The suggested/likely match is pre-highlighted at the top of the list (blue left-edge accent bar, not a fill — see Design system) so it's fast to confirm with one tap, but confirmation stays required — consistent with why store matching is user-driven search in the first place, not automatic detection.
...
* Override visual treatment (Aug 2026, superseded Sept 2026): fields where a store overrides the general default are now marked with solid Amber fill (bg `#FAEEDA`, text `#854F0B`) rather than light blue. The original blue treatment shared its accent with the store-search "likely match" highlight (see Store matching, below) and blended into the page background once that was corrected to `#E6F1FB`. Amber was chosen over a darker blue fill because Blue 400 is already the primary-action/navigation color — a solid blue block on a data row risked reading as a tappable element rather than an annotation. Tradeoff: Amber already meant "notes/warmth" (see Design system); to keep the two meanings from colliding in the same view, a store's custom/notes fields stay neutral rather than picking up the amber treatment. Verified Sept 2026: the Amber text-on-fill pairing measures 5.87:1, clearing the 4.5:1 threshold, so no deeper text tone is needed.
* Custom fields render neutral, not override-styled (Aug 2026): a store's custom fields (`sizes.custom`) sit in the same neutral styling as un-overridden core fields, not the amber override treatment — a custom field isn't correcting a general default (there isn't one to correct), it's purely additive, so giving it override styling would misrepresent what it's doing.
...
* Store subcategory grouping (Sept 2026): store categories without their own Core Sizes field group under a parent as nested rows. T-Shirts, Shirts / Button-downs, and Sweaters / Knitwear go under Tops; Shorts / Swim Trunks and Skirts go under Bottoms; Boots goes under Shoes. A subcategory never overwrites its parent, since a store can hold both Tops: S and T-Shirts: M. Nested rows render neutral (additive, same reasoning as custom fields) and hide along with their parent, counting toward the hidden-size notice. Swimwear and Jumpsuits / Rompers stay standalone in the "Also saved for [store]" section. The only full-body core field is Dresses, and filing men's swimwear under "Dresses" would read as gendered.
* Sizes view details (Sept 2026): an override row shows the user's usual size underneath in small text ("Usually M"), since amber alone says a size is different but not what it's different from; the line is omitted if no usual size is saved. A store entry that matches the usual size (ignoring spacing, capitalization, and x vs ×) renders as a normal row, not amber, and doesn't count toward the hidden-size notice, so amber stays reserved for real exceptions. Matching doesn't interpret meaning: "M" and "Medium" count as different. Store notes appear in full at the bottom of the "Also saved for [store]" card, in neutral styling, so sizes stay at the top. A store with only notes counts as having store data.
...
* Mirror source built and verified (Aug 2026): content script + background service worker implemented in `extension/`. Confirmed working end-to-end — visiting myfitfile.netlify.app triggers the content script to read localStorage, message it to the background worker, and write it into `chrome.storage.local` under `fitfile_mirror`. Verified via the extension's service worker console that real coreSizes, measurements, and stores data land correctly, with `syncStatus` populated (`state: "synced"`, `lastSyncedAt` timestamp, `error: null`).
* Implementation note: actual shape nests mirrored data one level deeper than originally spec'd — `fitfile_mirror.data.{coreSizes,measurements,stores}` rather than `fitfile_mirror.{coreSizes,measurements,stores}` directly. Functionally fine; anything reading from this later (popup, sync indicator) needs to reach through `.data` first. Left as-is rather than flattened, since it works and Claude Code's structure is reasonable.

**Sync architecture**

* Sync trigger (Aug 2026): sync fires on visiting the FitFile app itself, not on a timer or a manual popup button. A content script scoped to `https://myfitfile.netlify.app/*` (declared in `manifest.json` as `content-script.js`) reads the app's `localStorage` on page load and relays it to a background service worker (`background.js`), which writes it into `chrome.storage`. This is the only host-scoped permission the extension holds — store pages themselves are still handled through `activeTab` per the Permission scope decision above, so nothing runs on arbitrary sites.
* Remaining constraints (requirements gathered from decisions above, still to be worked out when building the sync code itself):
  * Must support an observable "syncing in progress" state and a distinct "sync failed / never synced" state — the popup UI already has copy and layout built for both (see Sync status states above). The background worker needs to expose that status (e.g. a status field in `chrome.storage`) so the popup isn't guessing.
